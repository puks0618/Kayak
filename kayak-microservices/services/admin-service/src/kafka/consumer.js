  ...
  if (!isConnected) {
    try {
      ...
      await consumer.subscribe({ 
        topics: ['flight-bookings', 'car-bookings'], 
        fromBeginning: false 
      });
      isConnected = true;
      console.log('✅ Admin Kafka Consumer connected to flight-bookings and car-bookings topics');
    } catch (error) {
...
const startConsuming = async () => {
  try {
    ...
    await consumer.run({
      eachMessage: async ({ topic, partition, message }) => {
        try {
          ...
          console.log(`📨 Admin received ${topic} event:`, {
          ...
          if (topic === 'flight-bookings') {
            switch (eventType) {
              ...
              case 'booking-status-updated':
                ...
                break;
              default:
                console.log('Unknown flight event type:', eventType);
            }
    ...
    console.log('🎧 Admin service listening for flight and car bookings...');
  } catch (error) {
...
const handleNewCarBooking = (bookingData) => {
  ...
  console.log(`✅ Admin processed car booking: ${bookingData.bookingId}`);
...
const handleCarBookingStatusUpdate = (bookingData) => {
  ...
  if (booking) {
    booking.status = bookingData.status;
    booking.updatedAt = bookingData.timestamp;
    console.log(`✅ Admin updated car booking status: ${bookingData.bookingId} -> ${bookingData.status}`);
  }
};
